...
//   board00[48], board01[48], ... board29[48], tape1[150], tape2[151]
...
static const uint16_t TAPE1_LEDS = 150;
static const uint16_t TAPE2_LEDS = 151;
static const uint16_t LEGACY_TOTAL = BOARD_TOTAL + TAPE1_LEDS;         // 1590
static const uint16_t TOTAL = BOARD_TOTAL + TAPE1_LEDS + TAPE2_LEDS;   // 1741
static const uint16_t LEGACY_FRAME_LEN = LEGACY_TOTAL * 3;             // 4770 bytes
static const uint16_t FRAME_LEN = TOTAL * 3;                           // 5223 bytes
...
static const uint8_t PIN_TAPE1 = 11;
static const uint8_t PIN_TAPE2 = 12;
...
Adafruit_NeoPixel stripTape1(TAPE1_LEDS, PIN_TAPE1, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel stripTape2(TAPE2_LEDS, PIN_TAPE2, NEO_GRB + NEO_KHZ800);
...
static void clearStrip(Adafruit_NeoPixel& strip, uint16_t ledCount) {
...
static void showBootPattern() {
  // 起動確認用: 各出力を色分けしつつ、最大輝度は 12 に統一
  for (uint16_t i = 0; i < LEDS_PER_ROW; i++) strip0.setPixelColor(i, strip0.Color(12, 0, 0));
  for (uint16_t i = 0; i < LEDS_PER_ROW; i++) strip1.setPixelColor(i, strip1.Color(12, 6, 0));
  for (uint16_t i = 0; i < LEDS_PER_ROW; i++) strip2.setPixelColor(i, strip2.Color(0, 12, 0));
  for (uint16_t i = 0; i < LEDS_PER_ROW; i++) strip3.setPixelColor(i, strip3.Color(0, 0, 12));
  ...
  strip3.show();
  strip4.show();
  for (uint16_t i = 0; i < TAPE1_LEDS; i++) stripTape1.setPixelColor(i, stripTape1.Color(0, 6, 12));
  for (uint16_t i = 0; i < TAPE2_LEDS; i++) stripTape2.setPixelColor(i, stripTape2.Color(12, 12, 0));
  stripTape1.show();
  stripTape2.show();
}
...
// row3: LEDs [864 .. 1151]
// row4: LEDs [1152 .. 1439]
static void applyFrame(const uint8_t* rgb, uint16_t payloadLen) {
  const uint32_t rowBytes = (uint32_t)LEDS_PER_ROW * 3U; // 864
  const uint32_t boardBytes = rowBytes * ROWS;
  const uint32_t tape1Bytes = (uint32_t)TAPE1_LEDS * 3U;
  const uint32_t tape2Offset = boardBytes + tape1Bytes;
  ...
  applyRowToStrip(stripTape1, rgb + boardBytes, TAPE1_LEDS);

  if (payloadLen >= FRAME_LEN) {
    applyRowToStrip(stripTape2, rgb + tape2Offset, TAPE2_LEDS);
  } else {
    clearStrip(stripTape2, TAPE2_LEDS);
  }
}
...
void loop() {
  ...
  while (Serial.available() >= 2) {
    ...
    if (len != FRAME_LEN && len != LEGACY_FRAME_LEN) {
      if (len > 8192) return; // 安全策
      discardBytes(len);
      continue;
    }

    if (!readExact(backBuf, len)) {
      // 途中受信は破棄
      continue;
    }
