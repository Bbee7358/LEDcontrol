...
static const uint16_t LEGACY_FRAME_LEN = BOARD_TOTAL * 3;             // board only fallback
...
static void applyFrame(const uint8_t* rgb, uint16_t frameLen) {
  ...
  if (frameLen >= boardBytes + (uint32_t)TAPE1_LEDS * 3U) {
    applyRowToStrip(stripTape1, rgb + boardBytes, TAPE1_LEDS);
  } else {
    clearStrip(stripTape1, TAPE1_LEDS);
  }

  if (frameLen >= boardBytes + (uint32_t)(TAPE1_LEDS + TAPE2_LEDS) * 3U) {
    applyRowToStrip(stripTape2, rgb + boardBytes + (uint32_t)TAPE1_LEDS * 3U, TAPE2_LEDS);
  } else {
