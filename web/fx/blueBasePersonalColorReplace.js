const BUILD_TAG = "blueBasePersonalColorReplace / 2026-03-10 JST";

function clamp255(v) {
  return v < 0 ? 0 : v > 255 ? 255 : (v | 0);
}

function hexToRgb(hex) {
  if (typeof hex !== "string") return { r: 0, g: 255, b: 136 };
  let s = hex.trim();
  if (s.startsWith("#")) s = s.slice(1);
  if (s.length === 3) s = s.split("").map(ch => ch + ch).join("");
  if (!/^[0-9a-fA-F]{6}$/.test(s)) return { r: 0, g: 255, b: 136 };
  const n = parseInt(s, 16);
  return {
    r: (n >> 16) & 255,
    g: (n >> 8) & 255,
    b: n & 255,
  };
}

export default {
  id: "blueBasePersonalColorReplace",
  label: "Blue Base + Personal Color Replace",
  desc: "全域を青10で点灯し、人の半径500mmだけベース青を消してその人色だけを描く最小版",
  params: [
    { key: "baseBlue", label: "Base Blue", type: "range", min: 0, max: 64, step: 1, default: 10 },
    { key: "personRadiusMm", label: "Person Radius(mm)", type: "range", min: 50, max: 1500, step: 10, default: 500 },
    { key: "personColor", label: "Fallback Person Color", type: "color", default: "#00ff88" }
  ],

  init(state, params) {
    state.logged = false;
  },

  render(ctx, out, state, params, geo) {
    const { TOTAL, worldX, worldY } = geo;

    if (!state.logged) {
      console.log("[FX BUILD]", BUILD_TAG);
      window.__FX_BUILD__ = BUILD_TAG;
      state.logged = true;
    }

    const baseBlue = clamp255(Number(params.baseBlue) || 10);
    const radius = Math.max(1, Number(params.personRadiusMm) || 500);
    const people = Array.isArray(ctx.people) ? ctx.people : [];
    const fallbackColor = hexToRgb(params.personColor || "#00ff88");

    // 1) まず全域を青で塗る
    for (let gi = 0; gi < TOTAL; gi++) {
      const k = gi * 3;
      out[k + 0] = 0;
      out[k + 1] = 0;
      out[k + 2] = baseBlue;
    }

    // 2) 人の半径内だけはベース青を完全に消し、その人色だけを描く
    for (let gi = 0; gi < TOTAL; gi++) {
      const x = worldX[gi];
      const y = worldY[gi];
      let hitColor = null;

      for (let i = 0; i < people.length; i++) {
        const p = people[i] || {};
        const px = Number(p.x);
        const py = Number(p.y);
        if (!Number.isFinite(px) || !Number.isFinite(py)) continue;

        const dx = x - px;
        const dy = y - py;
        const d = Math.hypot(dx, dy);
        if (d <= radius) {
          hitColor = hexToRgb(p.color || params.personColor || "#00ff88");
          break;
        }
      }

      if (hitColor) {
        const k = gi * 3;
        out[k + 0] = hitColor.r;
        out[k + 1] = hitColor.g;
        out[k + 2] = hitColor.b;
      }
    }

    // 3) 人データが無いときの確認用に fallback は使わない
    //    ただし params の color は、各 person.color 未指定時のみ使う
    void fallbackColor;
  }
};
